...
// ==================== FETCH ENTRIES FROM GITHUB API ====================
async function loadAllData() {
    const container = document.getElementById('entriesContainer');
    container.innerHTML = `
        <div class="loading-state">
            <div class="spinner"></div>
            <p>Loading entries from GitHub…</p>
        </div>`;
    try {
        ...
        container.innerHTML = `
            <div class="empty-state">
                ...
                <p>Could not load data from GitHub.<br><small>${err.message}</small></p>
            </div>`;
        console.error(err);
    }
}
...
async function deleteEntry(entryId, category) {
    try {
        ...
        showToast('🗑️ Entry deleted permanently', 'success');
...
function renderEntries(period) {
    ...
    container.innerHTML = filtered.map(entry => {
        ...
        // Determine icon and category name
        let icon = '📦';
        ...
        if (entry.inOut === 'in') {
            ...
            inOutClass = 'inout-out';
            inOutText = 'OUT';
        } else if (entry.inOut === 'absent') {
            inOutClass = 'inout-absent';
            inOutText = 'ABSENT';
        }
        ...
        return `
            <div class="entry-card">
                ...
                <div class="entry-main">
                    <span class="entry-category">${categoryName} ${inOutText ? `<span class="entry-inout ${inOutClass}">${inOutText}</span>` : ''}</span>
